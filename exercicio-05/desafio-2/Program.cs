Console.WriteLine("========= Exercício 5 - Desafio 2 =========");
...
var crates = new List<char>[cratesSize];
...
foreach (var command in commands)
{
    ...
    var charMoved = crates[fromNumber].TakeLast(moveNumber).ToArray();

    crates[toNumber].AddRange(charMoved);
    
    for(var i = 0; i < charMoved.Length; i++)
    {
        crates[fromNumber].RemoveAt(crates[fromNumber].Count - 1);
    }
}

foreach (var crate in crates)
{
    topCrate.Add(crate.Last());
}
